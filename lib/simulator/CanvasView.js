...
export class CanvasView {
	...
	// Used to play game on a larger screen 
	chkboxProcessor(checkboxElem) {
	  if (checkboxElem.checked) {
		// Checked--set LargeBoard vars to perist		
		this.processLargeSize();				
	  } 
	  else{
	    // Unchecked--reset original vars for SmallBoard
		location.reload();
	  }
	}	
	
	// Used to play game on a larger screen 
	addRobots(addRobotsElem) {
	  if (addRobotsElem.checked) {
		// Checked--call additional Robots process		
		// var Robots = [robot1, robot2];	
		this.robot = window.simulator.getRobotArray(3);
	  }
		this.processSize(this.robot);
	}
		
	processLargeSize() {
	    // LARGE GRID DIMENSIONS	
		this.robot = window.simulator.getCurrentLargeRobot();
	...
	processSize() {	
		...
		//this.robot = window.simulator.getNewRobot();
		this.canvas = document.getElementById("c");
		this.context = this.canvas.getContext("2d");		
  	    this.maxX = 5; // x total
		this.maxY = 5; // y total
	...
	// Set up a loop in the event 'robot' is an Array().
	renderRobot() {	
		// if chkAddRobots is not checked do this:
		this.chkbox = document.getElementById('chkRobots');	
		if(!document.getElementById("chkRobots").checked)
		{	
			var robot = window.simulator.getCurrentRobot(),
		...
		{	// if chkAddRobots is Checked deal with Array inputs...
			var robot = this.robot,
			robotAxisX = (robot[1].x + 1) * 100, // the center of the destination grid horizontally
			robotAxisY = (this.maxY - robot[1].y) * 100; // the center of the destination grid vertically
			var robot = robot[1];
		}
